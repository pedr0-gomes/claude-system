...
- **Aprender → virar skill** *(desenho fechado 2026-05-30; gatilho: rodar a esteira na mão uma vez)* — a esteira interna (fontes → instruções → NotebookLM → consolidação → mapa) nunca girou na mão. Quando houver um tema real pra aprender, rodar o fluxo ad-hoc — lendo a "Esteira interna do Aprender" deste arquivo por caminho absoluto, já que o bloco não é global — e só então destilar em skill se valer. **NotebookLM instalado em 2026-06-02** via `notebooklm-py` (CLI + skill em `~/.claude/skills/notebooklm/`; autenticado; `uv` em `~/.local/bin`). O bloqueio de infraestrutura desse bloco está resolvido — o que falta é rodar a esteira.
